...
class UserServiceTest {
    ...
    void update_ShouldReturnUpdatedUser_WhenUserExists() {
        ...
        when(repo.findById("1")).thenReturn(Optional.of(user1));
        when(repo.save(any(User.class))).thenReturn(updatedInfo); // Mocking save to return the modified object logic if needed or just argument

        // The service modifies the existing object and saves it. 
        // We should ensure the save method is called with the updated properties.
